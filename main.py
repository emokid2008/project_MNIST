...
# подсчет уникальных классов (целевых данных)
...
for i in range(40):
    axes = ax[i // 10, i % 10] # разбитие диапазона range(40) на двумерный массив [row, col]
    axes.imshow(data_train[i], cmap = 'gray') # отрисовка картинок с цифрами
...
sns.heatmap(conf_matrix, # матрица, на основе которой рисуем график
            annot = True, # заголовки столбцов и строк(числа от 0 - 9)
            fmt = 'd', # формат чисел (d - целые числа)
            ax = graph, # рисуем график в области graph
            cmap = 'Blues', # синяя цветовая схема (градиент от белого к темному(мало значений -> много значений))
...
for i in range(20):
    ax = axes[ i // 5, i % 5]
    ax.imshow(data_test[i], cmap = 'gray') # отрисовка картины
...
for index, error_idx in enumerate(worst_error):
    ...
    ax.imshow(data_test[error_idx], cmap = 'gray') # отрисовка картины
